import 'package:flutter/material.dart';

class AppKeys {
  static final homeScaffoldKey = GlobalKey<ScaffoldState>();
  static final registerScreenScaffoldKey = GlobalKey<ScaffoldState>();
  static final loginScreenScaffoldKey = GlobalKey<ScaffoldState>();
  static final forgetPasswordScreenScaffoldKey = GlobalKey<ScaffoldState>();
  static final verificationCodeScreenScaffoldKey = GlobalKey<ScaffoldState>();
  static final newPasswordScreenScaffoldKey = GlobalKey<ScaffoldState>();
  static final addLocationScreenScaffoldKey = GlobalKey<ScaffoldState>();
  static final confirmOrderScreenScaffoldKey = GlobalKey<ScaffoldState>();
  static final editProfileScreenScaffoldKey = GlobalKey<ScaffoldState>();
}
